...
*Note: You can use `0. Hyperparameter Optimization.py` to run an automated hyperparameter search. It uses **Hyperband (successive halving)** rather than exhaustive random search: 27 unique configurations start with a tiny budget of 4 training episodes, are deterministically evaluated, and only the best third survive to the next round with a larger budget (15, then 48, then the full 100 episodes for the single winner). Each stage is logged to `hyperband_run_<n>.csv` with rank, budget, score, and promotion status, and each run's winner is saved as `best_config_run_<n>.json` / `best_dqn_model_run_<n>.pth`. By default 3 independent searches run in parallel (one Godot instance per port, selectable via `--rl-port` on the Godot side and `--base-port` on the Python side) and their winners are compared in `hyperband_summary.csv`; the overall best config/weights are also written to `best_config.json` / `best_dqn_model.pth`.*
